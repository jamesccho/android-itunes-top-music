package io.github.jamesccho.itunestopmusic.data.util;
...
public class FeedParser {
    ...
    public List<Music> parse(InputStream in)
    ...
    private List<Music> readFeed(XmlPullParser parser)
            throws XmlPullParserException, IOException, ParseException {
        List<Music> entries = new ArrayList<>();
     ...
     */
    private Music readEntry(XmlPullParser parser)
            throws XmlPullParserException, IOException, ParseException {
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            if (tagName.equals(TAG_ID)) {
                id = readAttrValue(parser, TAG_ID, "im:id");
//                Log.d(TAG, id);
            } else if (tagName.equals(TAG_NAME)){
                name = readBasicTag(parser, TAG_NAME);
//                Log.d(TAG, name);
            } else if (tagName.equals(TAG_ARTIST)){
                artist = readBasicTag(parser, TAG_ARTIST);
//                Log.d(TAG, artist);
            }  else if (tagName.equals(TAG_IMAGE)){
                image = readBasicTag(parser, TAG_IMAGE);
//                Log.d(TAG, image);
            } else {
                skip(parser);
            }
        }
        return new Music(id, name, artist, image);
    }
    ...
    private String readAttrValue(XmlPullParser parser, String tag, String attr) throws IOException, XmlPullParserException {
        parser.require(XmlPullParser.START_TAG, ns, tag);
